  ...
  export class CartRenderer {
    ...
    handleMinusClick(btn, product) {
      ...
      if (currentQty > 1) {
        ...
        this.updateItemTotal(product.element, product.total);
        this.updateCartTotals();
        
        // Update localStorage
        this.storageManager.updateProductQuantity(product.id, currentQty);
      }
    }
    ...
    updateCartTotals() {
      let subtotal = 0;
      document.querySelectorAll('.cart-item').forEach((item) => {
        const price = parseFloat(item.querySelector('.product-price').textContent.split(' ')[0]);
